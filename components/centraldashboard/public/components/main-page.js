...
export class MainPage extends utilitiesMixin(PolymerElement) {
    ...
    _onEnvInfoResponse(responseEvent) {
        ...
        if (this.userrole !== 'admin') {
            const defaultNs =
            this.queryParams.ns ||
            ( // Ngược lại, tính biểu thức trong ngoặc sau:
                this.namespaces && //    – Nếu this.namespaces tồn tại
                // (không phải undefined/null),
                this.namespaces.length > 0 && //    – và mảng không rỗng,
                this.namespaces[0].namespace //
            ) ||
            ''; // Nếu cả hai bước trên đều falsy, mặc định gán chuỗi rỗng
